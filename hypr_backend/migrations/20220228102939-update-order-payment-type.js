...
exports.up = async db => {
  ...
  ('RBAC', 'P_UPDATE_ORDER_PAYMENT_TYPE_RBAC', '/api/v1/order/updateOrderPaymentType', 'Put', now(), now());`, []);
  await db.runSql(`INSERT INTO role_permissions (role_id, permission_id, created_at, updated_at) VALUES 
  (8, (SELECT id from permissions where code = 'P_UPDATE_ORDER_PAYMENT_TYPE_RBAC'), now(), now());`, []);
  return Promise.resolve();
};

exports.down = async db => {
  await db.runSql("SET SQL_SAFE_UPDATES = 0;");
  await db.runSql(`DELETE FROM permissions WHERE code = 'P_UPDATE_ORDER_PAYMENT_TYPE_RBAC';`, []);
