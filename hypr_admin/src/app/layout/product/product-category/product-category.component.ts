...
export class ProductCategoryComponent implements OnInit {
    ...
    updateSelectedLanguage(category) {
        category.selectedLanguage = this.languages.find(language => language.code == category.selectedLanguageCode);
        let languageExists = [];
        languageExists = category.multilingual.filter(language => language.language == category.selectedLanguageCode);
        if (languageExists.length == 0) {
            let categoryLanguage = { language: category.selectedLanguageCode, value: '', attributeName: attributes.name };
            if (category.id) {
                categoryLanguage['categoryId'] = category.id;
            }
            category.multilingual.push(categoryLanguage);
        }
    }
    ngOnInit() {
        this.getlocations();
    ...
    createCategory() {
        ...
        if (this.cat.name == "") {
            this.toastr.error("Please add category (L1) name");
            return;
        }
        if (!this.isValidCatName(this.cat.name)) {
            this.toastr.error("Category (L1) name must not contain all dots");
            return;
        }
        ...
        if (isDuplicate) {
            this.toastr.error("Sub Categories cannot have same name!");
            return;
        }
        ...
        if (this.cat.sub_categories.length > 0) {
            for (var sub_cat of this.cat.sub_categories) {
                ...
                if (sub_cat.name == "") {
                    this.toastr.error("Please add category (L2) name");
                    return;
                }
                if (!this.isValidCatName(sub_cat.name)) {
                    this.toastr.error(
                        "Category (L2) name must not contain all dots"
                    );
                    return;
                }
                ...
                delete sub_cat.selectedLanguageCode;
                delete sub_cat.selectedLanguage;
                sub_cat['multilingual'] = sub_cat['multilingual'].filter(language => language.value !== ''); // Remove empty languages
            }
        }
        delete this.cat['selectedLanguageCode'];
        delete this.cat['selectedLanguage'];
        this.cat['multilingual'] = this.cat['multilingual'].filter(language => language.value !== ''); // Remove empty languages

        this.sharedFunctions
            .postRequest("/categories/createProductCategory", this.cat)
            .subscribe(
                (data) => {
                    this.toastr.success("Created category successfully");
                    this.refresh();
                },
                (error) => {
                    this.toastr.error(error.error.message);
    ...
    updateCategory(category) {
        ...
        if (category.name == "") {
            this.toastr.error("Category name missing!");
            return;
        }
        if (!category.priority) {
            this.toastr.error("Category (L1) priority missing!");
            return;
        }
        ...
        if (category.sub_categories) {
            ...
            category.sub_categories.forEach((cat) => {
                ...
                delete cat.selectedLanguageCode;
                delete cat.selectedLanguage;
                cat['multilingual'] = cat['multilingual'].filter(language => !(language.value == '' && !language.createdAt)); // Remove empty languages
            });
            ...
            if (isNameEmpty) {
                this.toastr.error("sub-category name missing!");
                return;
            }
            if (isNameInValid) {
                this.toastr.error(
            ...
            if (priorityMissing) {
                this.toastr.error(
                    "Category (L2) priority missing!"
                );
                return;
            }
            if (maxPriorityError) {
                this.toastr.error(
                    "CATEGORY (L2) MAX PRIORITY " + maxPriority + " ALLOWED"
                );
                return;
            }
            if (minPriorityError) {
                this.toastr.error(
                    "CATEGORY (L2) MIN PRIORITY " + 1 + " ALLOWED"
                );
                return;
            }
        }
        delete category.selectedLanguageCode;
        delete category.selectedLanguage;
        category['multilingual'] = category['multilingual'].filter(language => !(language.value == '' && !language.created_at)); // Remove empty languages
        this.sharedFunctions
            ...
            .subscribe(
                (data) => {
                    ...
                    if (data.success) {
                        ...
                        this.clearLanguageFile();
                        this.updateCategoryLanguages(data.data.name);
                    }
                },
                (err) => {
                    this.loading = false;
                    this.toastr.error("Failed to uploaded file");
                }
            );
    }

    updateCategoryLanguages(data) {
        const obj = {
            fileName: data,
        ...
        this.sharedFunctions
            .putRequest("/api/v1/category/multilingual", obj)
            .subscribe((data) => {
                this.toastr.info("Category languages updating in progress");
            });
    }

    clearLanguageFile() {
        document.getElementById('category-language')['value']= null;
        this.file = null;
    }
}